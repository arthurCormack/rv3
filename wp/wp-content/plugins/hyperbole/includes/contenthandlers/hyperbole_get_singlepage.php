...
function hyperbole_get_singlepage($slug) {
  // return "hyperbole_get_singlepage: {$slug}";
  // $somePage = get_page_by_path($slug, OBJECT, array('page', 'post', 'zed_the_zoomer_book'));// is this returning the attachment?
  // url_to_postid
  $somePostID = url_to_postid($slug);
  if ($somePostID === 0) {
    return false;
  }
  $somePage = get_post($somePostID);
  // if ($somePage=== null) {
  //   return false;
  // }
