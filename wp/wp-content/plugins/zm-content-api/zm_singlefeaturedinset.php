...
function zm_singlefeaturedinset() {
  //Get the page that has the title "Home"
  $homePage = get_page_by_path( 'Home' );
  $someFeaturedSingleInset = get_field('ez2_single_featured_inset', $homePage->ID);
  // return $someFeaturedSingleInset;
  $someFeaturedSingleInsetStuff = array();
  foreach($someFeaturedSingleInset as $someFeaturedThingID) {
    ...
    // $someObj->post_content = apply_filters( 'the_content', $somePost->post_content);
        ...
        $someFeaturedVideo = get_field('featured_video', $someFeaturedThingID);
        $someObj->post_video = $someFeaturedVideo;

    array_push($someFeaturedSingleInsetStuff, $someObj);
  }
  return $someFeaturedSingleInsetStuff;
